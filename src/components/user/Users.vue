<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 用户列表区 -->
       <!-- :data="userlist绑定数据源 border边框线 stripe隔行变色-->
      <el-table :data="userlist" border stripe>
        ...
        <el-table-column label="状态">
          <!-- 定义了一个作用域插槽，通过slot-scope接收当前作用域的数据 -->
          <template slot-scope="scope">
            <el-switch v-model="scope.row.mg_state" @change="userStateChanged(scope.row)"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="操作">
          <!-- 该命令所在行的下一行的代码规范被屏蔽检查了 -->
  ...
  methods: {
    async getUserList(){
      ...
      async userStateChanged(userinfo){
        ...
        // eslint-disable-line解构响应中的data赋值给res,${动态数据}
      ...
      // 展示编辑用户的对话框,通过scope.row.id传入用户id
      ...
      // 如果某个函数调用结果是返回值是promise，可以用async和await进行优化
